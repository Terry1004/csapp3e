...
/* Log unix-style error but do not exit program */
void unix_errorf(char *msg, ...)
{
    va_list args;
    va_start(args, msg);
...
/* Log Getaddrinfo-style error but do not exit program */
void gai_errorf(int code, char *msg, ...)
{
    va_list args;
    va_start(args, msg);
    vfprintf(stderr, msg, args);
...
/* Log application error but do not exit program */
void app_errorf(char* msg, ...)
{
    va_list args;
    va_start(args, msg);
    vfprintf(stderr, msg, args);
                 ...
                 const struct addrinfo *hints, struct addrinfo **res)
{
    ...
    if ((rc = getaddrinfo(node, service, hints, res)) != 0) 
        gai_errorf(rc, "Getaddrinfo error");
    return rc;
}
/* $end getaddrinfo */
                 ...
                 size_t hostlen, char *serv, size_t servlen, int flags)
{
    ...
    if ((rc = getnameinfo(sa, salen, host, hostlen, serv, 
                          servlen, flags)) != 0) 
        gai_errorf(rc, "Getnameinfo error");
    return rc;
}
...
ssize_t Rio_readlineb(rio_t *rp, void *usrbuf, size_t maxlen) 
{
    ...
    if ((rc = rio_readlineb(rp, usrbuf, maxlen)) < 0)
	    unix_errorf("Rio_readlineb error");
    return rc;
} 
...
int Open_clientfd(char *hostname, char *port) 
{
	...
	unix_errorf("Open_clientfd error");
    return rc;
}
...
int Open_listenfd(char *port) 
{
	...
	unix_errorf("Open_listenfd error");
    return rc;
}
